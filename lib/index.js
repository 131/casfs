...
class casfs {
  ...
  async mount(mountPath) {
    ...
    fuse.mount(this.mountPath, {
      ...
      destroy : (cb) => {
        logger.debug('destroy', arguments, this._closing);
        if(this._closing)
          this.close(true);
        cb();
      },
  ...
  close(destroyed) {
    logger.info("SHOULD SUTDOWN", this.mountPath, destroyed);
    if(!this.mountPath)
      return;

    if(destroyed && this._closing)
      return this._closing = this._closing.resolve(), null;

    let closing = this._closing = defer();
    this.inodes.close();
